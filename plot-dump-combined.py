...
def read(file_path):
    ...
    for program in sorted(data,key=lambda x:x['program']):
        #global PROGRAM_TO_SHOW
        #if not program['program'] in PROGRAM_TO_SHOW:
        #    continue
        pprint (program.keys())
...
def main():
	...
	_,overheads_sc,_ = read("binaries-sc/measurements.json")
        print 'OVERHEADS'
        #pprint(overheads)
        #exit(1)
	...
	for coverage in coverage_keys:
	    ...
	    ax_color = coverage_color[coverage]
            ...
            print 'OH coverage {} mean:{} std: {} median:{}'.format(coverage, np.mean(overheads[coverage][means_dic_name]), np.std(overheads[coverage][means_dic_name]),np.median(overheads[coverage][means_dic_name]))
            pprint (overheads[coverage][means_dic_name])
            print 'SC coverage {} mean:{} std: {} median:{}'.format(coverage, np.mean(overheads_sc[coverage][means_dic_name]), np.std(overheads_sc[coverage][means_dic_name]),np.median(overheads_sc[coverage][means_dic_name]))
            pprint (overheads_sc[coverage][means_dic_name])
	    ...
	    #print coverage," ind",len(columns), 'means',len(overheads[coverage][means_dic_name]),'std',len(overheads[coverage][stds_dic_name])
            means_arr = np.pad(np.array(overheads[coverage][means_dic_name]),(0,len(columns)-len(overheads[coverage][means_dic_name])),'constant')
            means_arr_sc = np.pad(np.array(overheads_sc[coverage][means_dic_name]),(0,len(columns)-len(overheads_sc[coverage][means_dic_name])),'constant')
            stds_arr = np.pad(np.array(overheads[coverage][stds_dic_name]),(0,len(columns)-len(overheads[coverage][stds_dic_name])),'constant')
            stds_arr_sc = np.pad(np.array(overheads_sc[coverage][stds_dic_name]),(0,len(columns)-len(overheads_sc[coverage][stds_dic_name])),'constant')
	    ...
	    #rects1 = ax.bar(columns,means_arr, width, color=ax_color, edgecolor='black', capsize=4, label=coverage+'%')#,tick_label=[coverage]*N)
	    ...
	    #rects2 = ax.bar(columns,means_arr_sc, width, color=ax_color, hatch='.', edgecolor='black', capsize=3, error_kw={'pickradius':2,'ecolor':'orange'}, yerr=stds_arr_sc)#,tick_label=[coverage]*N)
