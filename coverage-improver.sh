...
do
	...
	opt-3.9 -load $INPUT_DEP_PATH/libInputDependency.so -load $INPUT_DEP_PATH/libTransforms.so $bitcode -lib-config=$libconfig -strip-debug -clone-functions -extract-functions -transparent-cache -dependency-stats -dependency-stats-file=$output_dir/dependency.stats -clone-stats -clone-stats-file=$output_dir/clone.stats -extraction-stats -extraction-stats-file=$output_dir/extract.stats -o $output
	if [ $? -eq 0 ]; then
		echo 'OK Transform'
	else
		echo 'FAIL Transform'
